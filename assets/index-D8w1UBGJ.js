...
`,v=(e,t,n)=>{let r=e.length+t.length+n.length,i=e.reduce((e,t)=>e+t.stars,0)+t.reduce((e,t)=>e+t.stars,0)+n.reduce((e,t)=>e+t.stars,0),a=new Set(n.map(e=>e.language));return`
    <section class="stats" aria-label="Stats">
      <div class="stat"><div class="stat__value">${r}</div><div class="stat__label">开源仓库</div></div>
      <div class="stat"><div class="stat__value">${m(i)}</div><div class="stat__label">累计 Stars</div></div>
      <div class="stat"><div class="stat__value">${a.size}+</div><div class="stat__label">涉及语言</div></div>
    </section>
  `},y=e=>`
  <a class="card" href="${f(p(e.repo))}"
     target="_blank" rel="noopener noreferrer">
    <h3 class="card__title"><span>${f(e.name)}</span></h3>
    <div class="card__repo">${f(e.repo)}</div>
    <p class="card__desc">${f(e.description)}</p>
    <div class="card__meta">
      <span class="stars"><span class="stars__icon">★</span> ${f(m(e.stars))}</span>
      <span>GitHub</span>
    </div>
  </a>
`,b=e=>{let t=o[e.language]??`#888`;return`
    <a class="card" href="${f(p(e.repo))}"
       target="_blank" rel="noopener noreferrer">
      <h3 class="card__title"><span>${f(e.name)}</span></h3>
      <div class="card__repo">${f(e.repo)}</div>
      ...
      <div class="card__meta">
        <span class="lang">
          <span class="lang__dot" style="background:${f(t)}"></span>
          ${f(e.language)}
        </span>
        <span class="stars"><span class="stars__icon">★</span> ${f(m(e.stars))}</span>
      </div>
...
`,S=e=>`
    <footer class="footer">
      <span>© ${new Date().getFullYear()} ${f(e.name)}</span>
      <span class="beian">
...
`,w=()=>`
  <div class="container">
    ...
    ${_(e)}
    ${v(t,n,[...r,...i,...a])}
    ${x(`Eclipse Plugins`,t,y)}
    ${x(`VS Code Extensions`,n,y)}
    ${x(`Desktop UI Projects`,r,b,!0)}
    ${x(`Web / Tools`,i,b,!0)}
    ${x(`Others`,a,b,!0)}
    ${S(e)}
  </div>
...
`,T=()=>{try{let e=localStorage.getItem(c);if(e===`light`||e===`dark`)return e}catch{}return window.matchMedia(`(prefers-color-scheme: dark)`).matches?`dark`:`light`},E=e=>{document.documentElement.setAttribute(`data-theme`,e);let t=document.getElementById(`theme-toggle`);t&&(t.innerHTML=h(e===`dark`?l:u,!0))},D=()=>{let e=T();E(e),document.getElementById(`theme-toggle`)?.addEventListener(`click`,()=>{e=e===`dark`?`light`:`dark`;try{localStorage.setItem(c,e)}catch{}E(e)})},O=()=>{let e=document.getElementById(`back-to-top`);if(!e)return;let t=()=>{e.hidden=!(document.documentElement.scrollHeight>window.innerHeight)};e.addEventListener(`click`,()=>{window.scrollTo({top:0,behavior:`smooth`})}),window.addEventListener(`scroll`,t,{passive:!0}),window.addEventListener(`resize`,t),t()},k=()=>{let e=document.getElementById(`app`);e&&(e.innerHTML=w(),D(),O())};document.readyState===`loading`?document.addEventListener(`DOMContentLoaded`,k):k();