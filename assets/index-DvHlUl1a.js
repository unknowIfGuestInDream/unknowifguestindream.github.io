...
`,D=()=>{try{let e=localStorage.getItem(u);if(e===`light`||e===`dark`)return e}catch{}return window.matchMedia(`(prefers-color-scheme: dark)`).matches?`dark`:`light`},O=e=>{document.documentElement.setAttribute(`data-theme`,e);let t=document.getElementById(`theme-toggle`);t&&(t.innerHTML=_(e===`dark`?d:f,!0))},k=()=>{let e=D();O(e),document.getElementById(`theme-toggle`)?.addEventListener(`click`,()=>{e=e===`dark`?`light`:`dark`;try{localStorage.setItem(u,e)}catch{}O(e)})},A=()=>{let e=document.getElementById(`back-to-top`);if(!e)return;let t=()=>{let t=document.documentElement.scrollHeight>window.innerHeight;e.hidden=!t};e.addEventListener(`click`,()=>{window.scrollTo({top:0,behavior:`smooth`})}),window.addEventListener(`scroll`,t,{passive:!0}),window.addEventListener(`resize`,t),t()},j=()=>{let e=document.getElementById(`app`);e&&(e.innerHTML=E(),k(),A())};document.readyState===`loading`?document.addEventListener(`DOMContentLoaded`,j):j();