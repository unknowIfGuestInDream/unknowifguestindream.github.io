...
`,_=(e,t,n)=>{let r=e.length+t.length+n.length,i=e.reduce((e,t)=>e+t.stars,0)+t.reduce((e,t)=>e+t.stars,0)+n.reduce((e,t)=>e+t.stars,0),a=new Set(n.map(e=>e.language));return`
    <section class="stats" aria-label="Stats">
      <div class="stat"><div class="stat__value">${r}</div><div class="stat__label">开源仓库</div></div>
      <div class="stat"><div class="stat__value">${p(i)}</div><div class="stat__label">累计 Stars</div></div>
      <div class="stat"><div class="stat__value">${a.size}+</div><div class="stat__label">涉及语言</div></div>
    </section>
  `},v=e=>`
  <a class="card" href="${d(f(e.repo))}"
     target="_blank" rel="noopener noreferrer">
    <h3 class="card__title"><span>${d(e.name)}</span></h3>
    <div class="card__repo">${d(e.repo)}</div>
    <p class="card__desc">${d(e.description)}</p>
    <div class="card__meta">
      <span class="stars"><span class="stars__icon">★</span> ${d(p(e.stars))}</span>
      <span>GitHub</span>
    </div>
  </a>
`,y=e=>{let t=o[e.language]??`#888`;return`
    <a class="card" href="${d(f(e.repo))}"
       target="_blank" rel="noopener noreferrer">
      <h3 class="card__title"><span>${d(e.name)}</span></h3>
      <div class="card__repo">${d(e.repo)}</div>
      ...
      <div class="card__meta">
        <span class="lang">
          <span class="lang__dot" style="background:${d(t)}"></span>
          ${d(e.language)}
        </span>
        <span class="stars"><span class="stars__icon">★</span> ${d(p(e.stars))}</span>
      </div>
...
`,x=e=>`
    <footer class="footer">
      <span>© ${new Date().getFullYear()} ${d(e.name)}</span>
      <span class="beian">
...
`,C=()=>`
  <div class="container">
    ...
    ${g(e)}
    ${_(t,n,[...r,...i,...a])}
    ${b(`Eclipse Plugins`,t,v)}
    ${b(`VS Code Extensions`,n,v)}
    ${b(`Desktop UI Projects`,r,y,!0)}
    ${b(`Web / Tools`,i,y,!0)}
    ${b(`Others`,a,y,!0)}
    ${x(e)}
  </div>
`,w=()=>{try{let e=localStorage.getItem(c);if(e===`light`||e===`dark`)return e}catch{}return window.matchMedia(`(prefers-color-scheme: dark)`).matches?`dark`:`light`},T=e=>{document.documentElement.setAttribute(`data-theme`,e);let t=document.getElementById(`theme-toggle`);t&&(t.innerHTML=m(e===`dark`?l:u,!0))},E=()=>{let e=w();T(e),document.getElementById(`theme-toggle`)?.addEventListener(`click`,()=>{e=e===`dark`?`light`:`dark`;try{localStorage.setItem(c,e)}catch{}T(e)})},D=()=>{let e=document.getElementById(`app`);e&&(e.innerHTML=C(),E())};document.readyState===`loading`?document.addEventListener(`DOMContentLoaded`,D):D();